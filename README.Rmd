...
`compareSignatures()` is a front end to `OmicSignature::compare_omic_signatures()`: it takes the same arguments with the same defaults, and assembles the signature lists from the database (by id or name), from OmicSignature objects, or both:
...
Ids or names that do not exist, or that the account cannot see, are reported in a warning and left out; fetched signatures that share a name are told apart as `"name (id N)"`. The result is the `compare_omic_signatures()` list, so it can be passed to `OmicSignature::signature_similarity_heatmap()`.
