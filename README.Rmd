...
High-throughput studies produce a growing volume of **omic signatures** — the ranked or thresholded feature sets that come out of a differential analysis. Most of them end up in supplementary tables, with inconsistent metadata and no shared representation, so they are rarely reused and comparing a new result against prior work stays a manual job.

**SigRepo is a platform that treats signatures as first-class, reusable objects**: stored in a common representation with controlled metadata, searchable, shareable, and — importantly — analyzable in place rather than only downloadable.

This repository is the **R client**. It talks to a running [**SigRepo_Server**](https://github.com/montilab/SigRepo_Server) instance, which provides the MySQL database, the REST API, the web interface, and an MCP server for AI agents. You can use the client against [**our deployed server**](https://sigrepo.org){target="_blank"} or against your own.

Signatures are represented as R6 objects defined by [**OmicSignature**](https://github.com/montilab/OmicSignature){target="_blank"}, our in-house package (GPL-3), which pairs a curated feature set with its underlying differential-expression table and a controlled metadata vocabulary.
...
- Run gene set enrichment against MSigDB with [hypeR](https://github.com/montilab/hypeR){target="_blank"} (`runHypeR()`, `prepareHypeRSignatures()`).
...
Navigate to our [sigrepo.org](https://sigrepo.org){target="_blank"} portal to create an account. On the login page, click `"Register here!"` and fill out the registration form. You will receive an email when your account has been activated.
...
With [Claude Code](https://claude.com/claude-code){target="_blank"}:
...
- [Setting up your own SigRepo_Server](https://montilab.github.io/SigRepo_Server/articles/install_sigrepo.html){target="_blank"}

Questions, or want an account? <a href="mailto:sigrepo@bu.edu">Contact us</a>.
