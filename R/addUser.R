...
addUser <- function(
  ...
  purrr::walk(
    ...
    function(u){
      ...
      # comes from the connection; a literal `sigrepo` here broke seeding on any
      # database with another name (#245).
      grant_statements <- SigRepo::user_grant_statements(
        user_name = table$user_name[u],
        user_role = table$user_role[u],
