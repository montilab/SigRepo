...
# The examples below add, update and delete data in the database. Run them only
# when a dedicated test account is configured through SIGREPO_TEST_DB_USER and
# SIGREPO_TEST_DB_PASSWORD; otherwise show the code without running it.
knitr::opts_chunk$set(eval = base::nzchar(conn_handler$user) && base::nzchar(conn_handler$password))
```
...
There is nothing to configure. A term that is somebody's whole value matches
only that value; a term that is nobody's whole value matches every value
containing it.

```{r}
# "LLFS" is no signature's whole name, so this finds every name containing it.
SigRepo::searchSignature(conn_handler = conn_handler, signature_name = "LLFS")

# A whole name returns that one signature, not every name containing it.
SigRepo::searchSignature(
  conn_handler = conn_handler,
  signature_name = "Aging_Hs_PeripheralBlood_LLFS_Aging_MontiLab"
)

# "liver" is itself a sample type, so it does not drag in the several hundred
# sample types that merely contain the word. "liv" is not, so it does.
SigRepo::searchSignature(conn_handler = conn_handler, sample_type = "liver")
SigRepo::searchSignature(conn_handler = conn_handler, sample_type = "liv")
```

Terms are literal, not patterns: a `.` means a dot and a `%` means a percent
sign. Numbers and closed vocabularies (`signature_id`, `PMID`, `year`,
`has_difexp`, `direction_type`, `assay_type`) always match exactly, so
`year = 201` finds nothing rather than the whole of the 2010s.

If you need a query the arguments cannot express, the connection is yours:
`conn <- SigRepo::conn_init(conn_handler)` and then `DBI::dbGetQuery()`.
